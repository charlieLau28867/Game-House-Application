...
# for storing player's guess and the detail of the random variable
...
#status of room number and  player
class Player(object):
...
# creating thread lock for handling gameplay in each room
    ...
    def ClientAuthentication(self, client):
        ...
        while(CurrentPlayer.playerstatus != 0):
            ...
            if not self.ConnectionCheck(connection,CurrentPlayer,sockfd,msg):
                print(f"{CurrentPlayer.name} Disconnected")
                return
    ...
    def CommandHandle(self,msg,CurrentPlayer):
        ...
        if(msg[0] == "/list" and len(msg) == 1):
            ...
            try:
                ...
                return command
            if roomNumber < 0 or roomNumber > 9:
                return command
...
def main(argv):
    ...
    # limit the number of player to 20 and start the game
    sockser.listen(20)
